...
def train(P, epoch, model, criterion, optimizer, scheduler, loader, adversary, logger=None):
    ...
    for n, (images, labels) in enumerate(loader):
        ...
        if epoch >= P.RA_start:
            ...
            interpolation_images = interpolation_rate * images_pair + (1 - interpolation_rate) * images_adv
            interpolation_output_1 = model(interpolation_images)

            benign_outputs = model(images_pair)
            interpolation_output_2 = interpolation_rate * benign_outputs + (1 - interpolation_rate) * outputs_adv
