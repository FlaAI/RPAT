...
from adv_lib.mart import mart_loss, mart_loss_ra
...
def train(P, epoch, model, criterion, optimizer, scheduler, loader, adversary=None, logger=None):
    ...
    for n, (images, labels) in enumerate(loader):
        ...
        if epoch >= P.RA_start:
            loss_adv, loss_mart = mart_loss_ra(model, images, labels, optimizer, distance=P.distance,
                                               eps_iter=P.alpha, eps=P.epsilon, nb_iter=P.n_iters,
                                               beta=P.beta, clip_min=0, clip_max=1,
                                               interpolation_rate=P.RA_ip_rate, lam=P.lam)
        else:
